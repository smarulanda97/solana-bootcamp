...
describe("swap", async () => {
  ...
  it("Puts the tokens Alice offers into the vault when Alice makes an offer", async () => {
    ...
    assert(offerAccount.offeredTokenMint.equals(accounts.tokenMintA));
    assert(offerAccount.requestedTokenMint.equals(accounts.tokenMintB));
    assert(offerAccount.requestedTokenAmount.eq(tokenBWantedAmount));
